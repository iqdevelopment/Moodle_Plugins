...
$output .=
  ...
  $alteredHeader = $DB->get_field_sql('SELECT id FROM {bbb_attendance_headers} WHERE bigbluebuttonid = '.$bbbid.' AND courseid = '.$courseid.'');
      ...
      if(!empty($alteredHeader)){
        ...
        $output = $attendance->renderBBBAttendance();
        ...
        if($attendance->startTime < ($attendance->course->startdate - 3600) 
              OR $attendance->finishTime > ($attendance->course->enddate + 3600) 
              OR $attendance->course->enddate == 0
              OR  $attendance->startTime > ($attendance->course->enddate + 3600)){
                //print_r($attendance);


                  $output = $attendance->renderBasicAttendance();
