...
global $USER,$DB,$CFG;
//use classes\my_organization\my_organization;
...
$enabled = $DB->get_field_sql('SELECT value FROM {config_plugins} WHERE plugin = ? AND name = ?',array('local_my_organization','categories'));
$enabled = explode(",",$enabled);
$oneTimeEnabled = $DB->get_field_sql('SELECT value FROM {config_plugins} WHERE plugin = ? AND name = ?',array('local_my_organization','type_0'));
if( in_array($course->category,$enabled) AND $oneTimeEnabled){
   ...
   echo '<b>'.get_string('notification_not_allowed', 'local_my_organization').'</b>';
}
//print_r($obj);
//echo get_string('noPeriodicText','local_my_organization');
