...
class certificate_class{
    ...
    public $table = "simplecertificate_issues";
    public $tablefilters = '';
    public $columns;
    public $querry = "SELECT {simplecertificate_issues}.id AS cert_id,{simplecertificate_issues}.certificateid,{simplecertificate_issues}.userid AS userid,{simplecertificate_issues}.coursecode,{simplecertificate_issues}.coursecertnumber,{simplecertificate_issues}.nadpis,{simplecertificate_issues}.hours AS hodiny,{simplecertificate_issues}.poradove_cislo_kod,{simplecertificate_issues}.datum AS datum_vydani,{simplecertificate_issues}.timecreated AS realne_vystaveni,{simplecertificate}.course AS courseid,{course_categories}.name AS nazev_kategorie,{course}.category AS id_kategorie,{course}.fullname AS cely_nazev_kurzu,{course}.shortname AS short_code_kurzu,{course}.startdate AS zacatek_kurzu,{course}.enddate AS konec_kurzu,{user}.username,{user}.email,{user}.firstnamephonetic AS titul_pred,{user}.firstname AS jmeno,{user}.lastname AS prijmeni,{user}.lastnamephonetic AS titul_za,{user}.firstaccess AS prvni_pripojeni,{user}.lastaccess AS posledni_pripojeni FROM {simplecertificate_issues} INNER JOIN {simplecertificate} ON {simplecertificate}.id = {simplecertificate_issues}.certificateid INNER JOIN {course} ON {course}.id = {simplecertificate}.course INNER JOIN {course_categories} ON {course_categories}.id = {course}.category INNER JOIN {user} ON {user}.id = {simplecertificate_issues}.userid INNER JOIN {course_completions} ON {course_completions}.course = {course}.id AND {course_completions}.userid = {simplecertificate_issues}.userid";
    public $recipient;
    public $initialdate;
    ...
    {       
        ...
        $this->columns[] = array('id'=> "certificateBasicInfo", 'name'=> "id", 'value'=> "int");
        $this->columns[] = array('id'=> "certificateBasicInfo", 'name'=> "userid", 'value'=> "int");
        $this->columns[] = array('id'=> "certificateBasicInfo", 'name'=> "coursecode", 'value'=> "text");
        $this->columns[] = array('id'=> "certificateBasicInfo", 'name'=> "coursecertnumber", 'value'=> "int");
        $this->columns[] = array('id'=> "certificateBasicInfo", 'name'=> "nadpis", 'value'=> "text");
        $this->columns[] = array('id'=> "certificateBasicInfo", 'name'=> "hours", 'value'=> "int");
        $this->columns[] = array('id'=> "certificateBasicInfo", 'name'=> "poradove_cislo_kod", 'value'=> "text");
        $this->columns[] = array('id'=> "certificateBasicInfo", 'name'=> "datum", 'value'=> "text");
        $this->columns[] = array('id'=> "certificateBasicInfo", 'name'=> "time_created", 'value'=> "date");
    ...
    {
        $this->$tablefilters[] = array( 'filter'=> "startdate", 'data'=> "", 'querry'=> "(({course}.startdate > #data# AND {course}.category NOT IN (127,124,128) ) OR ({course}.category IN (127,124,128) AND {course_completions}.timecompleted > #data#)) AND {simplecertificate_issues}.certificatename  !=\"dummy cert\" ", 'type' => 'date');
        $this->$tablefilters[] = array( 'filter'=> "enddate", 'data'=> "", 'querry'=> "(({course}.enddate < #data# AND {course}.category NOT IN (127,124,128)) OR ({course}.category IN (127,124,128) AND {course_completions}.timecompleted < #data#))  AND {simplecertificate_issues}.certificatename  !=\"dummy cert\"", 'type' => 'date');
    }
