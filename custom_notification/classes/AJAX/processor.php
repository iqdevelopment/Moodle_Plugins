...
namespace local_custom_notification\AJAX;
...
class processor{
    ...
    $text = $POST['text'];
    $sql = 'INSERT INTO {config_plugins} (plugin,name,value) values ("local_custom_notification",?,?)';
    $sqlOverwiteAll = 'UPDATE {custom_notification} SET text = ? WHERE courseid IN (SELECT id FROM {course} WHERE category = ?) AND type > 0 ';
        ...
        return get_string('template_saved','local_custom_notification');
    }
    ...
    {   global $DB;
        ...
        $DB->execute($sql,array('local_custom_notification','category_id-'.$id));
        return get_string('deleted','local_custom_notification');
    }
    ...
    {   global $DB;
       //print_r($POST);
      $existing = $DB->get_records_sql('SELECT * FROM {custom_notification} where courseid = ? AND type != 0',array($POST['courseid']));
      $array = array(1,3,7);

      if($existing){
        $sql = 'UPDATE {custom_notification} 
                    SET timecreated = ? ,
                        enabled = ? ,
                        text = ?
        ...
        return get_string('updated','local_custom_notification');
    }
    ...
    {   global $DB,$USER;
        ...
        $logQuerry = 'INSERT INTO {custom_notification_log} (notificationid,sent,userid,sender_userid,timecreated,text) values(?,?,?,?,?,?)';
    ...
    {   global $DB,$CFG;
        if(!$this->course){
            $this->course = $DB->get_record_sql('SELECT * FROM {course} WHERE id = ?',array($this->courseid)); 
        }
    ...
    static public function getCourseCustomField_select($courseId,$fieldId){
       ...
       **********/
       ...
       {    global $DB;
           ...
           $summ = $event-$now;
           $summ = round($summ/86400);
          $daysToStart = $summ;

            if ($daysToStart == 1) {
                $returnstring = get_string('future','local_custom_notification').' '.$daysToStart.' '.get_string('one_day_text','local_custom_notification');

            
            } elseif ($daysToStart > 1 AND $daysToStart < 5) {
                $returnstring = get_string('future','local_custom_notification').' '.$daysToStart.' '.get_string('days_2-4','local_custom_notification');

            
            } elseif ($daysToStart > 4) {
                $returnstring = get_string('future','local_custom_notification').' '.$daysToStart.' '.get_string('days_5_plus','local_custom_notification');
            
            } elseif ($daysToStart == 0) {
                $returnstring = get_string('today','local_custom_notification');
            
            } elseif ($daysToStart < 0 AND $daysToStart < -2) {
                $returnstring = get_string('past','local_custom_notification').' '.$daysToStart.' '.get_string('one_day_past','local_custom_notification');
            
            } elseif ($daysToStart < -2) {
                $returnstring = get_string('past','local_custom_notification').' '.$daysToStart.' '.get_string('days_2-4','local_custom_notification');
            }
...
// when every email fot tomorrow is sent, the $courses[1] is unset so the for loop will get to the 3-days from now and so one and so one...so we can send 30 emails every cron
       ...
       {    global $DB;
            $processDaysCheck = array(1,3,7);
            foreach ($processDaysCheck as $key => $value) {
               $check = $DB->get_field_sql('SELECT value FROM {config_plugins} WHERE plugin = ? AND name = ?',array('local_custom_notification','type_'.$value));
               if ($check == 1) {
                $processDays[] = $value;
               }
            }
            ...
            $sql = 'SELECT {course}.id,{course}.category,{course}.startdate,{custom_notification}.text FROM {course}
                    INNER JOIN {custom_notification} ON {custom_notification}.courseid = {course}.id
                    WHERE
                            {course}.category IN ('.$allowed.') 
                        AND {custom_notification}.enabled = 1
                        AND {custom_notification}.sent = 0
                        AND {course}.startdate > ?
                        AND {course}.startdate < ?
                        AND {custom_notification}.type = ?
                        ';
       ...
       {    global $DB;
        ...
        $sql = 'SELECT * FROM {user} WHERE id IN 
                    ...
                    AND id NOT IN (
                        SELECT userid FROM {custom_notification_log} 
                            WHERE sent = 1 AND notificationid = (
                                SELECT id FROM {custom_notification} 
                                    WHERE courseid = ? AND type = ?
                            )
                    )
                    LIMIT 1
        ...
        if(empty($user)){
            $updateQuerry               = 'UPDATE {custom_notification} SET sent = 1 , timecreated = ? WHERE courseid = ? AND type = ?';
                                $DB->execute($updateQuerry,array(time(),$this->course->id,$this->type));
            return     TRUE;
        }
        $logQuerry = 'INSERT INTO {custom_notification_log} (notificationid,sent,userid,sender_userid,timecreated,text) values (?,?,?,?,?,?)';
        $notificationId = $DB->get_field_sql('SELECT id FROM {custom_notification} WHERE courseid = ? AND type = ?',array($this->course->id,$this->type));
        $DB->execute($logQuerry,array($notificationId,1,$user->id,0,time(),$notificationId));
        $subject = $this->replaceBracketString($DB->get_field_sql('SELECT value FROM {config_plugins} WHERE plugin = ? AND name = ?',array('local_custom_notification','default_subject')));                        
        $from = $DB->get_record_sql('SELECT * FROM {user} where id = ?',array(2));
       // $user = $DB->get_record_sql('SELECT * FROM {user} where id = ?',array(5));
       ...
       /**
        * enrol notification process
        */

        public function enrol_notification_process($type,$skipSearch = false)
        {   global $DB;
            $confing_sql = 'SELECT value FROM {config_plugins} WHERE plugin = ? AND name = ?';
            $special_categories = $DB->get_field_sql($confing_sql,array('local_custom_notification','special_enrol_notification_category'));
            $special_categories = \explode(',',$special_categories);
            $returnObject = new \stdClass();


            if(in_array($this->course->category,$special_categories)){
                $messageBody = $DB->get_field_sql($confing_sql,array('local_custom_notification',$type.'_special_text'));
                $returnObject->body = nl2br($this->replaceBracketString($messageBody));

                $messageSubject = $DB->get_field_sql($confing_sql,array('local_custom_notification',$type.'_special_subject'));
                $returnObject->subject = nl2br($this->replaceBracketString($messageSubject));
                return $returnObject;

            }else{
                $messageBody = $DB->get_field_sql($confing_sql,array('local_custom_notification',$type.'_text'));
                $returnObject->body = nl2br($this->replaceBracketString($messageBody));

                $messageSubject = $DB->get_field_sql($confing_sql,array('local_custom_notification',$type.'_subject'));
                $returnObject->subject = nl2br($this->replaceBracketString($messageSubject));
                return $returnObject;
            }
        }
